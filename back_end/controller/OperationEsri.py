...
class OperationEsri:
    ...
    def calcule_esri(self, table_name: str):
        conn = None
        try:
            ...
            countries_codes = {code: number for code, number in [
            ...
            def extract_between_teller_and_mcbc(local_ref):
                match = re.search(r'TELLER,([^,]+)\.MCBC', local_ref)
                

                return match.group(1) if match else None

            def extract_value_from_local_ref(local_ref, field):
                values = local_ref.split('|')
                if field in teller:
                    index = teller.index(field)
                    if index < len(values):
                        return values[index]
                return None


            def update_address_based_on_country(row):
                code = row.get('Code pays donneur d\'ordre')
                return country_addresses.get(code, 'Adresse inconnue')

            extracted_data = []
            for _, row in df.iterrows():
                local_ref = row['local_ref']
                row_data = {}
                for col, field in columns_mapping.items():
                    if col == "Type":
                        row_data[col] = extract_between_teller_and_mcbc(local_ref)
                    else:
                        row_data[col] = extract_value_from_local_ref(local_ref, field)

                row_data["Agence"] = row["Agence"]
                row_data["Montant"] = row["Montant"]
                row_data["Adresse donneur d'ordre"] = update_address_based_on_country(row_data)
                # Nettoyer Code pays
                code_pays = (row_data.get('Code pays donneur d\'ordre') or '').strip().upper()
                row_data['Code pays'] = countries_codes.get(code_pays)
                extracted_data.append(row_data)

            extracted_df = pd.DataFrame(extracted_data)

            # Supprimer les colonnes existantes inutiles et remplacer par extracted_df
            drop_cols = ['local_ref','Agence','Montant','Code pays donneur d\'ordre']
            df = df.drop(columns=[c for c in drop_cols if c in df.columns])
            combined_df = pd.concat([df, extracted_df], axis=1)
            ...
            if combined_df.empty:
                print("[WARN] combined_df est vide, vérifie le traitement")
            else:
                combined_df.to_sql(table_name, conn, if_exists='replace', index=False)

            # Écrire le résultat dans la table MySQL
            combined_df.to_sql(table_name, conn, if_exists='replace', index=False)
            print(f"[INFO] Extraction ESRI terminée pour {table_name} ✅")

        except Exception as e:
            print(f"[ERREUR] extractionEsri : {e}")
        finally:
            if conn:
                try:
                    conn.close()
                except Exception as close_err:
                    print(f"[ERREUR] Fermeture connexion (extractionEsri) : {close_err}")