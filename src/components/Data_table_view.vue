...
watch(usePopupStore().selected_date, (val) => {  
  ...
  fetchData(`http://192.168.1.73:8000/api/get_encours_credits?date=${val.value}`, listes.encours, 'encours_actual_data');
  fetchData(`http://192.168.1.73:8000/api/encours_remboursement?date=${val.value}`, listes.remboursement, 'remboursement_actual_data');
...
onMounted(() => {
  ...
  const date = '20250829';
  fetchData(`http://192.168.1.73:8000/api/get_encours_credits?date=${date}`, listes.encours, 'encours_actual_data');
  fetchData(`http://192.168.1.73:8000/api/encours_remboursement?date=${date}`, listes.remboursement, 'remboursement_actual_data');
  fetchData(`http://192.168.1.73:8000/api/encours_limit?limit_type=8400`, listes.avm, 'limit_avm_actual_data');
  fetchData(`http://192.168.1.73:8000/api/encours_limit?limit_type=2900`, listes.caution, 'limit_caution_actual_data');
});
</script>
<style scoped>
.bg_data {
