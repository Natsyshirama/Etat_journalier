...
const load_database = async (refresh, files, folder,date_string) => {
  ...
  var index_table=0;
  try {
    const response = await fetch('http://192.168.1.73:8000/api/create_multiple_table', {
      method: 'POST',
      headers: {
...
const uploadFile = async (folder_name) => {
  ...
  try {
    const response = await fetch('http://192.168.1.73:8000/api/upload_multiple_files', {
      method: 'POST',
      body: formData,
    });
